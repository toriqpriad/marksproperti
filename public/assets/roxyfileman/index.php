...
session_start();
$key = $_GET['key'];
$_SESSION['dir_upload'] = '/properti_ci/public/assets/images/backend/'.$key;
?>
<html>
<head>
<!--
...
window.location.replace("http://localhost/properti_ci/public/login");
} else {
			var input = JSON.stringify(storage);
			$.ajax({
            url: 'http://localhost/properti_ci/public/checkwebtoken',
            method: 'POST',
            data: input,
            dataType: 'json',
            cache: false,
